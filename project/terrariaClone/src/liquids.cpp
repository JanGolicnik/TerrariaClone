...
namespace liquids
{
	...
	void updateTile(glm::vec2 pos, std::vector<std::pair<int, int>>* toadd, Layer* bs) {
		...
		if (map.count(c) >= 1) {
			...
			int id = (data & 0x00ff00f0u) >> 16;
			int timer = (data & 0x000000f0u) >> 4;
			int level = data & 0x0000000fu;
			unsigned int dir = ((data & 0x0f000000u) >> 24);
			int max = (data & 0xf0000000u) >> 28;
			...
			if (Layers::canLiquidGoThru(bs, c + offset)) {
				if (map.count(c + offset) <= 0) {
					toadd->push_back({ c + offset, (data & ~0x0000000fu) | (unsigned int)ceil(level / 2.0f) });
	...
	void updateOnScreen()
	{
		...
		for (int x = 0; x < floor(Layers::blocksOnScreen.x/2.0f); x++) {
			for (int y = 0; y < Layers::blocksOnScreen.y; y++) {
				glm::vec2 pos = floor(camC + glm::vec2(x, y));
				updateTile(pos, &toadd, bs);
			}
		}
		for (int x = Layers::blocksOnScreen.x; x >= ceil(Layers::blocksOnScreen.x / 2.0f); x--) {
			for (int y = 0; y < Layers::blocksOnScreen.y; y++) {
				glm::vec2 pos = floor(camC + glm::vec2(x, y));
				updateTile(pos, &toadd, bs);
			}
