        ...
        if (input::shift) {
            ...
            if (input::pressed(k_INVENTORY)) {
                if (Player::dead)
                {
                    ...
                    UI::hideChildren(ECS::getComponent<uiC>(guideslot), state, true);
                    if (state) {
                        sounds::menuclose();
            ...
            if (input::rheld(GLFW_KEY_M)) {
                //enemies::spawnEnemy("tombstone", globals::mouseBlockCoords());
            }
            if (input::rheld(GLFW_KEY_L)) {
                liquids::place("water", globals::mouseBlockCoords(), 0x4);
            }
            if (input::rheld(GLFW_KEY_U)) {
                liquids::place("lava", globals::mouseBlockCoords(), 2);
            }
            if (input::rpressed(GLFW_KEY_K)) {
                liquids::settleAll();
            }
            if (input::rheld(GLFW_KEY_M)) {
                liquids::place("water", globals::mouseBlockCoords(), 0x8);
            }
            if (input::rheld(GLFW_KEY_N)) {
                liquids::clean();
            }
            if (input::rpressed(GLFW_KEY_J)) {
                game::droppedItemSys->dropItem(globals::mouseBlockCoords(false), "manacrystal", 1);
            }
