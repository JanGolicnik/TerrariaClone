...
namespace cam {
	...
	void updateTowardsPlayer()
	{
        ...
        if (c.x < (0 + Layers::blocksOnScreen.x / 2)) {
            pos.x = -(Layers::blocksOnScreen.x / 2);
        }
        if (c.x > map::mapX- Layers::blocksOnScreen.x / 2) {
            pos.x = -map::mapX + Layers::blocksOnScreen.x / 2;
        }
        if (c.y < 0 + Layers::blocksOnScreen.y / 2) {
            pos.y = -Layers::blocksOnScreen.y / 2;
        }
        if (c.y > map::mapY - Layers::blocksOnScreen.y / 2) {
            pos.y = -map::mapY + Layers::blocksOnScreen.y / 2;
        }
