...
namespace UI {
    ...
    void create()
    {
        hotbar = new Inventory({ 100, -100 }, { 10, 1 }, &PlayerHotbar, false, MainLoop::inventoryC);

        inventory = new Inventory({ inventoryPos.x, inventoryPos.y - 110 }, { 10, 4 }, &PlayerInventory, true, MainLoop::inventoryC);
        inventory->toggle(1);

        Inventory::setupInventory(&defaultChest, { 10,4 });
        chest = new Inventory({ chestInvPos, {10,4}, &defaultChest, true, MainLoop::chestInvC, 0.885f});
        openChest = -1;
    }
    ...
    void addElement(int entity, UIFunction func, glm::vec2 pos, glm::vec2 size, int parent, std::unordered_map<std::string, uiStat> stats, std::unordered_map<std::string, std::string> textStats, bool hidden, uiAnchor anchor)
    {
        ...
        case ui_TEXT: {
            ...
            addElement(ent, ui_EMPTY, c.pos, glm::vec2(0), entity, {}, {}, hidden, anchorNONE);
            ECS::getComponent<drawC>(ent)->textScale = stats["textSize"].floatVal;
        }
            break;
        case ui_TOOLTIP:
        {    uiStat centered; centered.boolVal = false;
        ...
        case ui_BACK: {
            ...
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, c.size, entity, { {"autocorrect", {.boolVal = false} } }, { {"tex", "backmiddle"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} } }, { {"tex", "backleft"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} } }, { {"tex", "backtop"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} }, {"flipX", {.boolVal = true} } }, { {"tex", "backleft"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} }, {"flipY", {.boolVal = true} } }, { {"tex", "backtop"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} } }, { {"tex", "backcorner"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false} }, {"flipX", {.boolVal = true} } }, { {"tex", "backcorner"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false}  }, {"flipY", {.boolVal = true} }, {"flipX", {.boolVal = true} } }, { {"tex", "backcorner"} }, hidden, anchor);
            addElement(ECS::newEntity(), ui_DISPLAY, c.pos, size, entity, { {"autocorrect", {.boolVal = false}  }, {"flipY", {.boolVal = true} } }, { {"tex", "backcorner"} }, hidden, anchor);
    ...
    void openInventory()
    {
        UI::hideChildren(ECS::getComponent<uiC>(MainLoop::inventoryC), false);
        UI::hideChildren(ECS::getComponent<uiC>(MainLoop::equipmentC), false);
        UI::inventory->toggle(0);
        UI::chest->toggle(0);
        ECS::getComponent<uiC>(MainLoop::crafting)->hidden = UI::inventory->hidden();
    };
    ...
    void itemslotONCLICK(uiCfunctionArguments)
    {
        ...
        auto citem = getStat(ECS::getComponent<uiC>(MainLoop::cursoritem), "item", { .itemp = nullptr })->itemp;
    ...
    void craftableitemONCLICK(uiCfunctionArguments)
    {
        ...
        if (c >= 0) {
            auto ci = ECS::getComponent<uiC>(MainLoop::cursoritem);
    ...
    void cursoritemONRIGHTCLICK(uiCfunctionArguments)
    {
        ...
        if (item != nullptr) {
            ...
            vel *= 0.5;
            if (DroppedItems::dropItem(Player::center.x, Player::center.y, item->item, item->num, false, vel)) {
                item->num = 0;
            }
        }
    }
    ...
    {
        auto item = getStat(p, "item")->itemp;
        auto tt = ECS::getComponent<uiC>(MainLoop::tooltip);
        if (item != nullptr) {
    ...
    void cursorONRENDER(uiCfunctionArguments)
    {
        ...
        p->pos = globals::mouseBlockCoords(false, false);
    ...
    void pickuptextONRENDER(uiCfunctionArguments)
    {
        ...
        std::string text = UI::getTStat(p, "text", "") + " (" + std::to_string(UI::getStat(p, "num", { .intVal = 0 })->intVal) + ")";
        draw->text = text;
    ...
    {
        auto textp = ECS::getComponent<uiC>(p->children[0]);
        ...
        p->size += UI::getStat(p, "padding", { .floatVal = 0 })->floatVal;
    ...
    {
        MainLoop::newMap = false;
        map::name.clear();
        if (map::load()) {
            Terraria::changeSceneTo(GAME);
        }
    }
    ...
    {
        MainLoop::toggleSettings();
    }

    void uiCfunc_openSettingsMenu(uiC* p)
    {
        MainLoop::openMenu(UI::getStat(p, "menu")->intVal);
    }

    void uiCfunc_saveAndExitGame(uiC* p)
    {
        if (map::save() && Player::save()) {
            Terraria::changeSceneTo(STARTMENU);
        }
        else {
    ...
    void uiCfunc_loadPlayer(uiC* p)
    {
        ...
        hotbar = new Inventory({ 100, -100 }, { 11, 1 }, &PlayerHotbar, false, MainLoop::inventoryC);

        Inventory::setupInventory(&PlayerInventory, { 11,6 });
        inventory = new Inventory({ inventoryPos.x, inventoryPos.y - 110 }, { 11, 6 }, &PlayerInventory, true, MainLoop::inventoryC);
        inventory->toggle(1);

        Inventory::setupInventory(&defaultChest, { 10,4 });
        chest = new Inventory({ chestInvPos, {10,4}, &defaultChest, true, MainLoop::chestInvC });
        openChest = -1;
    ...
    {
        Terraria::toggleFullscreen();
    }

    void uiCfunc_nextResolution(uiC* p)
    {
        Terraria::nextResolution();
    }
